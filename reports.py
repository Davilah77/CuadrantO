...
def export_schedule_pdf(module, monday):
    ...
    if logo_path and logo_path.is_file():
        content.append(Image(str(logo_path), width=18 * mm, height=18 * mm, kind="proportional"))
    title_style = ParagraphStyle("title", fontName="Helvetica-Bold", fontSize=15, alignment=TA_CENTER, spaceAfter=4)
    content.append(Paragraph(str(settings.get("app_name") or "CuadrantO"), title_style))
    content.append(Paragraph(f"Cuadrante del {monday:%d/%m/%Y} al {(monday + timedelta(days=6)):%d/%m/%Y}", title_style))
    ...
    if reminders:
        content.append(Spacer(1, 3 * mm))
        content.append(Paragraph("Recordatorios especiales", ParagraphStyle("reminder-title", fontName="Helvetica-Bold", fontSize=9)))
        for reminder in reminders:
            content.append(Paragraph(reminder, ParagraphStyle("reminder", fontName="Helvetica", fontSize=8, leading=10)))
